...
def reward_fun(self):
    angle_error = (self.th) % (2 * np.pi) - np.pi
    reward = norm.pdf(angle_error, loc=0, scale=1)*10
    penalty = 0.01 * reward * abs(self.omega)**2 
    
    
    self.cumulative_penalty += 0.001 * abs(self.u)
    penalty += self.cumulative_penalty

    th_penalty = 0.01 * abs(self.th) ** 2

    return reward - penalty - th_penalty
...
class UnbalancedDisk(gym.Env):
    ...
    th =            
           ...
           pi/2   ----- -pi/2
                    ...
                    0  = starting location
    '''
    def __init__(self, umax=3., dt = 0.025, render_mode='human', max_steps=5000):
        ############# start do not edit  ################
        ...
        self.steps = 0
        self.max_steps = max_steps #for reward function
    ...
    def step(self, action):
        ...
        step_limit = self.steps >= self.max_steps
        return self.get_obs(), reward, step_limit, False, {}
